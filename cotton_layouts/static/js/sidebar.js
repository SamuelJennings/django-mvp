(function () {
  ...
  // Get toggle breakpoint from .sidebar-layout classes
  function getToggleBreakpoint() {
    const sidebarLayout = document.querySelector('.sidebar-layout')
    if (!sidebarLayout) return 'lg' // default if no sidebar-layout found

    const breakpoints = ['sm', 'md', 'lg', 'xl', 'xxl']
    for (const bp of breakpoints) {
      if (sidebarLayout.classList.contains('hide-sidebar-' + bp)) {
        return bp
      }
    }
    return 'lg' // default
